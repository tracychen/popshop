...
type UpdateMinBalanceERC20FormData = z.infer<typeof schema>;

function UpdateMinBalanceERC20Action({
  symbol,
  decimals,
  strategyContract,
  refresh,
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<UpdateMinBalanceERC20FormData>({
  ...
  async function onSubmit(data: UpdateMinBalanceERC20FormData) {
    setIsSaving(true);
    try {
      toast({
        title: "Processing",
      ...
      <div className="grid gap-3">
        ...
        {Number(watch("minBalance")) > 0 && (
          <p className="px-1 text-xs text-muted-foreground">
            {`Buyers will need ${formatUnits(
              BigInt(watch("minBalance")),
              decimals,
            )} ${symbol} to get this discount.`}
          </p>
        )}
        {errors.minBalance && (
...
export const getUpdateMinBalanceERC20Action = ({
  symbol,
  decimals,
  strategyContract,
  refresh,
  ...
  return {
    ...
    tabDescription:
      "Update the number of tokens that buyers will need to get a discount.",
    tabContent: (
      <UpdateMinBalanceERC20Action
        symbol={symbol}
        decimals={decimals}
